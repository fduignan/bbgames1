...
#define BLOCKCOUNT 30
  ...
  while (Level > 0)
  {
    ...
    while (!LevelComplete)
    {
      ...
      if (Ball.getY() >= Bat.getY() + Bat.getHeight() + 10)
      {
        BallCount--;
        if (BallCount == 0)
        {
...
#define MAX_ATTACKER_MISSILES (MAX_INVADERS)
  ...
  while (Level > 0)
  {
    ...
    Invaders[0].move(23, 3);
    Invaders[1].move(43, 3);
    Invaders[2].move(63, 3);
    Invaders[3].move(83, 3);
    Invaders[4].move(103, 3);
    Invaders[5].move(123, 3);
    Invaders[6].move(143, 3);
    Invaders[7].move(163, 3);
    Invaders[8].move(183, 3);
    Invaders[9].move(183, 3);
    ...
    Screen.putText("Level", 5, 5, 300, RGBToWord(0xff, 0xff, 0xff), RGBToWord(0, 0, 0));
    Screen.putNumber(MAX_INVADER_LEVELS - Level + 1, 60, 300, RGBToWord(0xff, 0xff, 0xff), RGBToWord(0, 0, 0));
    GameOver = 0;
    ...
    while (!LevelComplete )
    {
      ...
      {
        if ((AttackerMissiles[AttackerMissileCounter].getY() < 290) && (AttackerMissiles[AttackerMissileCounter].visible()))
        {
